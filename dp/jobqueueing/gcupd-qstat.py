"""
Script to update the generate climos queue using qstat.

qstat is called for each submitted job in the queue, and the result is used to update the queue entry for that job.
"""
...
def qstat_to_yaml(string):
    """Convert the output of `qstat -f -1` to a YAML-compliant string so it can easily be parsed. """
...
def update_from_qstat_item(session, entry, qstat_item):
    ...
    if comp_time and entry.status == 'RUNNING':
        ...
        entry.completion_message = yaml.dump(qstat_item, default_flow_style=False)
