...
def basename_components(filepath):
    ...
    return os.path.basename(filepath).split('_')
...
@mark.parametrize('input_file, climo_files', [
    ('gcm', ('gcm', False, t_start(1965), t_end(1970))),
    ('gcm', ('gcm', True, t_start(1965), t_end(1970))),
], indirect=['input_file', 'climo_files'])
def test_filenames(input_file, climo_files):
...
@mark.parametrize('input_file, climo_files, t_start, t_end', [
    ('gcm', ('gcm', False, t_start(1965), t_end(1970)), t_start(1965), t_end(1970)),
], indirect=['input_file', 'climo_files'])
def test_climo_metadata(input_file, climo_files, t_start, t_end):
    ...
    for fp in climo_files:
        with CFDataset(fp) as cf:
            ...
            assert cf.climo_tracking_id == getattr(input_file, 'tracking_id', '')
...
def test_varnames(input_file, climo_files):
...
@mark.parametrize('input_file, climo_files, climo_year', [
    ('gcm', ('gcm', False, t_start(1965), t_end(1970)), 1968),
], indirect=['input_file', 'climo_files'])
def test_time_var(input_file, climo_files, climo_year):
